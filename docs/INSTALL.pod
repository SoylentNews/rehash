...
MySQL AB, so we would recommend that you upgrade, though as far as we
know it still works fine.  At some point in the future we will switch
over to some syntaxes which have been recommended for some time and this
will break on 4.0.x, so please upgrade to at least 4.1.x whenever you get
a chance (you probably have until 2007 before we spring this on you).
...
and 5.0.22 for some time with no problems.
