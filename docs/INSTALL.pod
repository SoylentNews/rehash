...
use 2.2.6.  And while you probably do not want to use the very latest
...
We know you want to get right into the installation, but you need to
read this whole document first.  Notably, the software and hardware
requirements are given in the L<"REQUIREMENTS"> section, which you should
go read now, to make sure you're not wasting your time.  Don't forget
to come back here when you're done!

Also in this file, you will find information on upgrading a Slash site
(which can be tricky);  uninstalling;  and troubleshooting for common
problems.
...
drop, index and alter.  For example, if your whole site (slashd
daemon, apache, and mysql server) will run on the same machine, and
you wanted to use the mysql username 'slash', you might:

	GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER
	ON slash.* TO 'slash'@'localhost' IDENTIFIED BY (quoted passwd);
...
port, and then the mysql username and password you GRANTed privileges to.

If you don't understand what you're doing here, don't fake it; this is
a common reason we see for Slash installations failing.
...
coded test suites, which do not allow for changed but still-legitimate
output.  When this happens, the module itself is fine but it will not
install unless forced.  As of August 2006, we've noticed the following
modules throw spurious errors:
...
and as of this writing (July 2006), Slashdot has been running on 5.0.18
...
On Debian Linux, or Debian-based distributions like Ubuntu, try:
...
libperl-dev.  And on Debian stable, try libmysqlclient12 (and -dev)
for 4.0.x and/or libmysqlclient14 (and -dev) for 4.1.x.  On testing or
unstable, try libmysqlclient15off and libmysqlclient15-dev.
...
T_2_5_0_x and R_2_5_0_x (though there was no 2.5.0 release).
...
in step 5, in which case you have a serious enough deficit in
instruction-following that you might want to start over and be more
careful this time.
...
at the command line and see if it emits the perl binaries you
expect and the location of Slash.pm that you expect.  Check permissions
and see "Multiple Perls installed" below.
...
When you installed the Blob plugin, the script told you to check to see
if there were any plugin README files, right?  You thought we were
kidding, didn't you?  Go read plugins/Blob/README.

=item *

DBD::mysql will not install.

As of August 2006, its tests by default assume you have a running mysqld
on localhost, accessible to the user 'root' with no password.  If you
don't run mysqld on the same machine as you're installing the module,
or if you have a password for its 'root' user (good idea), almost all
its tests will fail.  You can either override the failures with
...
seclev (maybe to 100) in users.pl, go back to the command line, run
...
installing 5.6.1, say.  But if your C</usr/bin/perl> is a different
