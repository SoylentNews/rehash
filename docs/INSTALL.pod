...
B<NOTE>: As of December 2004, you'll see installation errors saying that
the table type of InnoDB doesn't support FULLTEXT indexes.  This is
a known bug that would be too complicated to properly work around at
the moment (sorry).  The real fix for it is coming soon.  For now, your
best bet is to issue the following commands after installation finishes:

	ALTER TABLE comments TYPE=MyISAM;
	ALTER TABLE comment_text TYPE=MyISAM;
	ALTER TABLE pollquestions TYPE=MyISAM;
	ALTER TABLE story_text TYPE=MyISAM;
...
Version 4.0.26 (4.0.12).

	http://www.mysql.com/

MySQL 3.23.x is no longer supported, as of CVS tag T_2_5_0_33
(October 18, 2004).  MySQL 4.1.x has been tested and we haven't found
any problems, but we haven't run it on a production site yet.
...
we wouldn't recommend running an earlier version.
