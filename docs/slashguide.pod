...
the regime of themes and plugins
	Slash::Install makes it happen
	themes can inherit from other themes
	plugins modify themes by adding htdocs files, adding tasks, adding templates,
		changing schemas, 
basic page layout
	left column
	middle column
	right column (usually present, sometimes not)
	differences between slashcode and slashdot themes (xoxox vs. xxoxo, mainly)
installation
	symlinks and side effects of using them
	template-tool and symlink-tool
	my preferred method of customizing a theme (cp -a themes/foo themes/bar)
	the other way (the slashdot theme way) of customizing a theme (override slashcode where necessary)
Templates
	1;2;3 and search order
	how template-tool lets themes override plugins (needed rarely, but sometimes)
	hard-rendering
		Slash/Slash.pm:sub _hard_dispComment {
		Slash/Utility/Display/Display.pm:sub _hard_linkStory {
		Slash/Utility/Display/Display.pm:sub _hard_linkComment {
	speed and RAM concerns
Blocks
	static, portald, and color
Vars
	getCurrentStatic = constants
The other quasi-globals
	getCurrentDB
	getCurrentUser
	getCurrentForm
	and the lesser-knowns like getCurrentAnonymousCoward
...
MySQL replication
	...
	fulltext indexes, (only) necessary on search db which should be a slave
