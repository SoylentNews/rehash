...
sub add_entry {
	...
	if ($id) {
		my $newnick = Slash::Client::fixparam($self->{nick});
		return($id, "$self->{http}://$self->{host}/~$newnick/journal/$id");
	}
...
Returns the unique ID of the new entry, or false if failure.
...
Returns the unique ID of the modified entry, or false if failure.
