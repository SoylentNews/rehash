...
sub createSubmission {
	...
	# We don't want to insert blank submissions into the DB. Are filters
	# applied to submissions, if not, there should be a few rexeps here to
	# do away with whitespace filled fields.
	...
	$submission->{'-time'} = 'now()' unless $submission->{'time'};

	# To help cut down on duplicates generated by automated routines. For crap-
	# flooders, we will need to look into an alternate methods.  Filters of 
	# some sort, maybe?
	$submission->{'signature'} = md5_hex($submission->{story});
