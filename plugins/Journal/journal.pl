...
sub saveArticle {
printf STDERR "00:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
...
printf STDERR "01:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
	# from comments.pl
...
printf STDERR "02:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');

	return 0 unless _validFormkey($ws ? qw(max_post_check interval_check) : ());

printf STDERR "03:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
	my $slashdb = getCurrentDB();
	if ($form->{id}) {
printf STDERR "04:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
...
printf STDERR "05:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
...
printf STDERR "06:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
...
printf STDERR "08:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
		unless ($form->{comments_on}) {
			for (qw(article tid posttype)) {
				$update{$_} = $form->{$_} if defined $form->{$_};
			}
			$update{description} = $description;
printf STDERR "09:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
		}

		$journal->set($form->{id}, \%update);
printf STDERR "10:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');

		return $form->{id} if $ws;
		$form = { id => $form->{id} };

	} else {
printf STDERR "11:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
		my $id = $journal->create($description,
			$form->{article}, $form->{posttype}, $form->{tid});

		unless ($id) {
printf STDERR "12:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
			unless ($ws) {
				_printHead("mainhead") or return;
...
printf STDERR "13:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
...
printf STDERR "14:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
			$journal->set($id, { discussion => $did });
		}
printf STDERR "15:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
...
printf STDERR "16:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
...
printf STDERR "17:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
		}
...
printf STDERR "18:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
	displayArticle($journal, $constants, $user, $form, $reader);
printf STDERR "19:%d:journal:currentPage :: ", $$, getCurrentUser('currentPage'), getCurrentUser('uid');
}
