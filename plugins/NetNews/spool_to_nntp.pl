...
sub generateUUCPBatch {
	...
	for (@$story_data) {
		...
		$batch = slashDisplay("netnews_story",
			{ story => \%story, urlize => \&urlize, absolutedir => $absolutedir },
			{ Return => 1, Nocomm => 1, Page => 'netnews', Skin => 'NONE' }
		);
		...
		for (@$comments) {
			...
			$comment{ref_count} = scalar @ref; #subtract 1 so if the array is empty its zero
			$batch = slashDisplay("netnews_comment",
				{ comment => \%comment, urlize => \&urlize, absolutedir => $absolutedir },
				{ Return => 1, Nocomm => 1, Page => 'netnews', Skin => 'NONE' }
			);
