...
$task{$me}{code} = sub {
	...
	if ($dirty_skins{$mp_skid} || $w ne "ok") {
		...
		if ($constants->{plugin}{FireHose}) {
			gen_firehose_static($virtual_user, "index_firehose.shtml", $gSkin->{name}, "", {  skipmenu => 1, skippop => 1, fhfilter=> "story", duration => "7", mode => 'fulltitle', mixedmode => '1', color => "black"  }); 
			gen_firehose_static($virtual_user, "firehose.shtml", $gSkin->{name}, "", { duration => "7", mode => 'fulltile', mixedmode => 1, color => "blue", nodates => '1'  }); 
		}
	...
	if ($do_all) {
		for my $key (sort { $a <=> $b } keys %dirty_skins) {
			...
			if ($constants->{plugin}{FireHose}) {
				gen_firehose_static($virtual_user, "index_firehose.shtml", $skin->{name}, $skinname, { skipmenu => 1, skippop => 1, fhfilter=> "'story $skin->{name}'", duration => "7", mode => 'fulltitle', mixedmode => '1', color => "black"  }); 
				gen_firehose_static($virtual_user, "firehose.shtml", $skin->{name}, $skinname, { duration => "7", mode => 'fulltitle', mixedmode => '1', color => "blue", nodates => '1', fhfilter => "'$skin->{name}'" }); 
			}
